...
def execute_bump_hack(c):
    ...
    2. git add pyproject.toml [other, files]
    ...
    if release_type == "none":
        ...
        c.run(
            f'standard-version --commit-all --release-as {release_type} --releaseCommitMessageFormat "bump: ✈️ {previous_tag} → v{{{{currentTag}}}}"',  # noqa: E501
            pty=True,
        )
        # push to origin
        c.run("git push --follow-tags origin main", pty=True)
    else:
...
@task(
    help={
        ...
        "year": "the year for which you'd like to create a directory tree (defaults to current year if not specified)",
        "help": "show this help message and exit",
    }
)
def run(c, version=False, year=None, help=False):
    """run the program"""

    if version:
        c.run("python hello_flit/cli.py -v", pty=True)
    elif year:
        c.run(f"python hello_flit/cli.py {year}", pty=True)
    elif help:
        c.run("python hello_flit/cli.py -h", pty=True)
    else:
        c.run("python hello_flit/cli.py", pty=True)
...
def lint(c, fix=False):
    ...
    if fix:
        ...
        c.run("flake8 tests hello_flit", pty=True)
